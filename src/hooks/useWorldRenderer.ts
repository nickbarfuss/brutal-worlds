...
import {
  ...
  Expanse,
  IntroPhase,
  GamePhase} from '@/types/game';
import {
  mainNebulaVertexShader,
  mainNebulaFragmentShader,
  wispyNebulaVertexShader,
  wispyNebulaFragmentShader,
...
const handleHighlightMeshes = (
  ...
  const {
    commandFillMesh: newCommandFill,
    commandBorderMeshes: newCommandBorders,
    highlightFillMesh: newHighlightFill,
    highlightBorderMeshes: newHighlightBorders,
...
export const useWorldRenderer = (props: MapRendererProps) => {
  ...
  useEffect(() => {
    ...
    const mainNebulaMaterial = new THREE.ShaderMaterial({
      uniforms: {
        color: { value: new THREE.Color(worldNebulaConfig.main.color) },
        density: { value: worldNebulaConfig.main.density * 6.0 },
        opacity: { value: 0.0 },
      },
      vertexShader: mainNebulaVertexShader,
      fragmentShader: mainNebulaFragmentShader,
      blending: THREE.AdditiveBlending,
      transparent: true,
      depthWrite: false,
    });
    const wispyNebulaMaterial = new THREE.ShaderMaterial({
      uniforms: {
        color: { value: new THREE.Color(worldNebulaConfig.wispy.color) },
        density: { value: worldNebulaConfig.wispy.density * 6.0 },
        falloff: { value: worldNebulaConfig.wispy.falloff },
        opacity: { value: 0.0 },
      },
      vertexShader: wispyNebulaVertexShader,
      fragmentShader: wispyNebulaFragmentShader,
      blending: THREE.AdditiveBlending,
      transparent: true,
      depthWrite: false,
    });
    for (let i = 0; i < 12; i++) {
      ...
      plane.position.copy(
        new THREE.Vector3(
          ...
          (Math.random() - 0.5),
          (Math.random() - 0.5)
        ).normalize().multiplyScalar(SPHERE_RADIUS * 180 * (1 + (Math.random() - 0.5) * 0.2))
      );
